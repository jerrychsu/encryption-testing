...
module.exports = {
    initialize: function( currentScript, scriptVersion ) {
        ...
        this.EXTERNAL_ID = clientOptionsObject.externalid || null;
...
module.exports.isOptedOutViaGDPRConsent = async function() {
    return new Promise( ( resolve ) => {
        if ( typeof window.__cmp !== 'function' ) {
            return resolve( false );
        }

        window.__cmp( 'getVendorConsents', c.CONSENT_PURPOSE_IDS, ( consent, success ) => {
...
const clientOptions = __webpack_require__( 0 ).initialize( currentScript, "1.5.0-newcjs-hits-stag-id-service" ); // eslint-disable-line no-undef
// Main object
...
module.exports.main = async function() {
    ...
    // Catch supressed errors
    utils.addEvent( window, 'error', function errorListener( errorEvent ) {
        let scriptSource = utils.fullyDecodeURI( clientOptions.SOURCE, 'scriptSource' );
        let filename = utils.fullyDecodeURI( errorEvent.filename, 'filename', false );

        if ( filename && scriptSource && filename.indexOf( scriptSource ) > -1 ) {
            errorEvent.preventDefault();
            errorEvent.stopImmediatePropagation();
            utils.reportError( 'suppress', errorEvent.error );
        }
    } );
